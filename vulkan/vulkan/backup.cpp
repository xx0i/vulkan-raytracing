...
private:
	...
	void createRayTracingDescriptorSetLayout()
	{
		...
		{
			topLevelASBinding, outputImageLayoutBinding, uboLayoutBinding
		};
	...
	void createShaderBindingTables()
	{
		...
		GetRayTracingShaderGroupHandlesKHR(device, rayTracingPipeline, 0, groupCount, shaderHandleStorage.size(), shaderHandleStorage.data());

		createBuffer(sbtSize, VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, shaderBindingTableBuffer, shaderBindingTableBufferMemory);
		...
		raygenRegion.deviceAddress = shaderBindingTableAddress;
	...
	{
		createImage(swapChainExtent.width, swapChainExtent.height, 1, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_STORAGE_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, storeImage, storeImageMemory);

		transitionImageLayout(storeImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, 1);
	}
	...
	void createAlphaImage()
	{
		...
		transitionImageLayout(alphaImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, 1);
	}
	...
	{
		storeImageView = createImageView(storeImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}

	void createAlphaImageView()
	{
		alphaImageView = createImageView(alphaImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}
	...
	void createBLAS()
	{
		...
		triangleData.vertexStride = sizeof(float) * 3;
	...
	void recordRayTracingCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, rayTracingPipelineLayout, 0, static_cast<uint32_t>(descriptorSetsToBind.size()),
		...
		transitionImageLayout(swapChainImages[imageIndex], swapChainImageFormat, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, 1);
